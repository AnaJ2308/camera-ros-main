...
// I will add flag that reads when topics from all cameras are received, then only update heightfield
...
constexpr float base = 0.005f;  // base offset in MuJoCo
...
constexpr double WINDOW_S = 0.15;  // 150 ms freshness window
constexpr size_t NUM_CAMS = sizeof(CAM_TOPICS)/sizeof(CAM_TOPICS[0]);
// =========================== MuJoCo helpers ===========================
...
R"XML(<mujoco model='pc2_live_hfield'>
  ...
  <asset>
    <hfield name='terrain' nrow='160' ncol='160' size='6.0 6.0 0.8 0.005'/>
  </asset>
  <worldbody>
...
// added this for per-camera buffer
    const float neg_inf = -std::numeric_limits<float>::infinity();
    for (size_t i = 0; i < NUM_CAMS; ++i) {
        grid_cam_[i].assign(NROW * NCOL, neg_inf);
        grid_cam_prev_[i].assign(NROW * NCOL, neg_inf);
        last_seen_[i] = std::chrono::steady_clock::time_point::min();
    }

    for (size_t i = 0; i < NUM_CAMS; ++i) {
        const char* topic = CAM_TOPICS[i];
      subs_.push_back(this->create_subscription<sensor_msgs::msg::PointCloud2>(
        topic, qos,
        [this, topic, i](const sensor_msgs::msg::PointCloud2::SharedPtr msg){
          this->callback(msg, topic, i);
        }));
      RCLCPP_INFO(this->get_logger(), "Subscribed to %s", topic);
    }

    // periodic on_tick @ 62.5 Hz
    timer_ = this->create_wall_timer(
      std::chrono::milliseconds(25), // do 16 for 62.5 hz, use 25 for 30hz
  ...
  void on_tick() {
    // Freshness window gate: all cameras must be recent enough
    const auto now = std::chrono::steady_clock::now();
    bool all_fresh = true;
    {
        std::lock_guard<std::mutex> sl(seen_mtx_);
        for (size_t i = 0; i < NUM_CAMS; ++i) {
            if (last_seen_[i] == std::chrono::steady_clock::time_point::min() ||
                now - last_seen_[i] > std::chrono::duration<double>(WINDOW_S)) {
    ...
    const size_t N = static_cast<size_t>(NROW) * NCOL;
    const float neg_inf = -std::numeric_limits<float>::infinity();

    std::vector<float> fused; fused.resize(N, neg_inf);
    {
    std::lock_guard<std::mutex> lk(mtx_);
    for (size_t idx = 0; idx < N; ++idx) {
      float m = neg_inf;
      for (size_t i = 0; i < NUM_CAMS; ++i)
        m = std::max(m, grid_cam_[i][idx]);
      fused[idx] = std::isfinite(m) ? m : -1.0;  // fill empty
    }
    //  Normalize to [0,1]
    ...
    hfield_dirty_.store(true, std::memory_order_relaxed);
    }

    // Copy meters grid - old code
    ...
    // Push to MuJoCo and reset raw meters grid - old code. not per-camera buffer
    // {
    //   std::lock_guard<std::mutex> lk(mtx_);
...
private:
  void callback(const sensor_msgs::msg::PointCloud2::SharedPtr& msg, const char* topic, size_t cam_idx) {
    ...
    {
    std::lock_guard<std::mutex> sl(seen_mtx_);
    last_seen_[cam_idx] = std::chrono::steady_clock::now();
    }
    ...
    {
      std::lock_guard<std::mutex> lk(mtx_);
    //   added this for ghost issue
        std::fill(grid_cam_[cam_idx].begin(),
            grid_cam_[cam_idx].end(),
            -std::numeric_limits<float>::infinity());
      fuse_points_to_grid_meters(pts_xyz, grid_cam_[cam_idx]);
      
      // low-pass filtering
      auto& curr = grid_cam_[cam_idx];       // current per-camera grid (meters)
      auto& prev = grid_cam_prev_[cam_idx];  // previous per-camera grid (meters)
      const float a = 0.3f;
       
      for(size_t k = 0; k < curr.size(); ++k) {
     
        const bool c_ok = std::isfinite(curr[k]);
        const bool p_ok = std::isfinite(prev[k]);

        if (c_ok && p_ok) {
            curr[k] = a * curr[k] + (1.0f - a) * prev[k];
        } else if (!c_ok && p_ok) {
            // no new sample for this cell from this camera: hold/decay as you prefer
            curr[k] = prev[k];
        }
        // if c_ok && !p_ok: keep curr as is
      ...
      new_frame_.store(true, std::memory_order_relaxed);
    }
  ...
  std::array<std::chrono::steady_clock::time_point, NUM_CAMS> last_seen_;
  std::mutex seen_mtx_;

  std::array<std::vector<float>, NUM_CAMS> grid_cam_; // per-camera NROW*NCOL
  const double expire_s_ = 0.5;

  std::array<std::vector<float>, NUM_CAMS> grid_cam_prev_;
...
int main(int argc, char** argv) {
  ...
  while (!glfwWindowShouldClose(win)) {
    ...
    mj_step(m, d);
    std::vector<float> staged; //copies heights01_stage_ into the local staged vector and clears the dirty flag.
